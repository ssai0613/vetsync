...
class AppointmentController
{
    ...
    public function handlePostRequest()
    {
        if (isset($_POST['action'])) {
            $responseMessage = "Error: Invalid action or incomplete data provided.";
            switch ($_POST['action']) {
                case 'add':
                    ...
                    if (empty($ownerId) || empty($petId) || empty($purpose) || empty($date) || empty($time)) {
                        ...
                        if ($result === true) {
                            ...
                            $responseMessage = "Error: The selected time slot is already occupied. Please choose another time or date.";
                        } else {
                            $responseMessage = "Failed to add appointment. Please try again or check the server logs.";
                        }
                    }
                    break;
                 ...
                 case 'complete':
                    ...
                    if ($appointmentId && isset($totalAmount) && isset($amountPaid) && isset($changeAmount)) {
                        if (!is_numeric($totalAmount) || !is_numeric($amountPaid) || !is_numeric($changeAmount) || floatval($amountPaid) < floatval($totalAmount)) {
                            $responseMessage = "Error: Invalid amounts or amount paid is less than total.";
                        } else {
                         ...
                         $responseMessage = "Error: Incomplete data for completing the appointment.";
                    }
                    break;
                default:

...
class DashboardModel {
    ...
    // Helper function to create a SQL date filtering condition.
    ...
    // Gets the details of pending appointments.
    ...
    // Gets total sales from products only.
    ...
    // NEW: Gets total revenue from services.
    ...
    // UPDATED: Gets the total count of all registered pets, ignoring any date filters.
    public function getTotalRegisteredPetsCount() {
        $query = "SELECT COUNT(DISTINCT pet_id) FROM pet";
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
        return $stmt->fetchColumn() ?? 0;
    }

    // Gets product sales data for the chart.
    ...
    // Gets service sales data for the chart.
